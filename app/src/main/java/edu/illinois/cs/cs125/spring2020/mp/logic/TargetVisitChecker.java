...
public class TargetVisitChecker {
    ...
    public static boolean isTargetWithinRange(final double[] latitudes, final double[] longitudes,
        ...
        double playerTargetDistance = LatLngUtils.distance(latitudes[targetIndex],
                longitudes[targetIndex], currentLatitude, currentLongitude);
        return playerTargetDistance <= range;
    }
    ...
    public static boolean checkSnakeRule(final double[] latitudes, final double[] longitudes,
        ...
        for (int i = 0; i <= count - 2; i++) {
            if (LineCrossDetector.linesCross(new LatLng(latitudes[path[i]], longitudes[path[i]]),
                    ...
                    new LatLng(latitudes[lastIndex], longitudes[lastIndex]),
