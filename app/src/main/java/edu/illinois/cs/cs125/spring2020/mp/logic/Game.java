...
public abstract class Game {
    ...
    public final int getWinningTeam() {
        ...
        int redTeam = getTeamScore(TeamID.TEAM_RED);
        int blueTeam = getTeamScore(TeamID.TEAM_BLUE);
        int greenTeam = getTeamScore(TeamID.TEAM_GREEN);
        int yellowTeam = getTeamScore(TeamID.TEAM_YELLOW);
        int max = redTeam;
        int winningTeam = TeamID.TEAM_RED;
        if (blueTeam > max) {
            max = blueTeam;
            winningTeam = TeamID.TEAM_BLUE;
        }
        if (greenTeam > max) {
            max = greenTeam;
            winningTeam = TeamID.TEAM_GREEN;
        }
        if (yellowTeam > max) {
            winningTeam = TeamID.TEAM_YELLOW;
            max = yellowTeam;
        }
        return winningTeam;
