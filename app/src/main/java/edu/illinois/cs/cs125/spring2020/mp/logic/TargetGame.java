...
public final class TargetGame extends Game {
    ...
    public void locationUpdated(final LatLng location) {
        ...
        for (Map.Entry<String, Target> entry : targets.entrySet()) {
            double playerTargetD = LatLngUtils.distance(location, entry.getValue().getPosition());
            if (playerTargetD <= proximityThreshold) {
    ...
    private void tryClaimTarget(final String id, final Target target) {
        ...
        List<String> targetVisited = playerPaths.get(getEmail());
        ...
        if (targetVisited.size() > 0) {
            String lastId = targetVisited.get(targetVisited.size() - 1);
            LatLng lastPosition = targets.get(lastId).getPosition();
            LatLng tryPosition = targets.get(id).getPosition();
            for (Map.Entry<String, List<String>> entry : playerPaths.entrySet()) {
                List<String> allVisited = entry.getValue();
                for (int i = 1; i < allVisited.size(); i++) {

                    LatLng linePointA = targets.get(allVisited.get(i - 1)).getPosition();
                    LatLng linePointB = targets.get(allVisited.get(i)).getPosition();
                    if (LineCrossDetector.linesCross(
                            lastPosition, tryPosition, linePointA, linePointB
                    )) {
                        return;
                    }
                }
            ...
            JsonObject targetVisit = new JsonObject();
            targetVisit.addProperty("type", "targetVisit");
            targetVisit.addProperty("targetId", id);
            sendMessage(targetVisit);
    ...
    private void addLineSegment(final LatLng start, final LatLng end, final int team) {
        ...
        final int[] teamColorArray = getContext().getResources().getIntArray(R.array.team_colors);
        PolylineOptions fill = new PolylineOptions().add(start, end)
                .color(teamColorArray[team]).width(lineThickness).zIndex(1);
